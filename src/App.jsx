...
export default function App() {
  ...
  const [formData, setFormData] = useState({ productName: '', targetPrice: '' })
  ...
  const fetchAvailableProducts = async () => {
    try {
      const res = await axios.get(`${API}/available-products`)
      setAvailableProducts(res.data.products || [])
    } catch (err) { console.log('Error fetching available products:', err) }
  }
  ...
  const handleAddProduct = async (e) => {
    e.preventDefault()
    if (!formData.productName || !formData.targetPrice) {
      setMessage('❌ Product aur Target Price select karo!')
      return
    }
    setLoading(true)
    try {
      ...
      setMessage('✅ Product track ho gaya! Dashboard mein dekho.')
      setFormData({ productName: '', targetPrice: '' })
      fetchProducts()
    } catch (err) {
        ...
        {page === 'add' && (
          <div className="max-w-xl mx-auto">
            <div className="bg-white rounded-xl shadow-lg p-8">
              <h2 className="text-2xl font-bold mb-6">📱 Product Track Karo</h2>
              <form onSubmit={handleAddProduct} className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">📦 {availableProducts.length} products available</p>
                </div>
                <div>
